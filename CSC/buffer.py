...
class Buffer:
    ...
    def init_tensors_herding(self, examples: torch.Tensor, labels: torch.Tensor,
                     logits: torch.Tensor, task_labels: torch.Tensor) -> None:
        """
        Initializes just the required tensors.
        :param examples: tensor containing the images
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None):
        ...
        for i in range(examples.shape[0]):
            ...
            print("num_seen_examples",self.num_seen_examples)
            if index >= 0:
    ...
    def add_data_herding(self, index, examples, labels=None, logits=None, task_labels=None):
        self.num_seen_examples += 1
        print("num_seen_examples",self.num_seen_examples)
        if not hasattr(self, 'examples'):
            self.init_tensors_herding(examples, labels, logits, task_labels)
        self.examples[index] = examples.to(self.device)
        if labels is not None:
            self.labels[index] = labels.to(self.device)
        if logits is not None:
            self.logits[index] = logits.to(self.device)
        if task_labels is not None:
            self.task_labels[index] = task_labels.to(self.device)
    ...
    def get_data(self, size: int, transform: nn.Module = None, return_index=False) -> Tuple:
        ...
        choice = np.random.choice(min(self.num_seen_examples, self.examples.shape[0]),
                                  size=int(size), replace=False)  # int
